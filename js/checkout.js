...
const checkMagiamgia =()=>{
    document.querySelector("#magiamgia")
    .addEventListener('change', (event) => {  // Use 'change' instead of 'onchange'
        console.log(event);

        const magiamgiaValue = {
            'MAGIAM10': 100000,
            'MAGIAM20': 200000
        };

        const magiamgia = event.target.value;
        if(magiamgia){
    return  errorElement.innerText = ''; 
        }
        const value = magiamgiaValue[magiamgia];

        const errorElement = document.querySelector("#magiamgia-error");

        if (value) {
            const price = Number(priceFinal.innerText.replace('đ','').replace('')) - value>0 ? Number(priceFinal.innerText) - value : 0;
            priceFinal.innerText = price
            errorElement.innerText = '';  // Use innerText to clear the error message
        } else {
...
document.querySelector("#complete-checkout-btn")
    .addEventListener('click', () => {
        let products = JSON.parse(localStorage.getItem('products'));
        xulyDathang(products);
    })
